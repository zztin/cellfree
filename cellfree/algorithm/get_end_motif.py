...
def get_fragment_neighbouring_sequence(molecule, extra_bp=4, reference=None):
    ...
    try:
        span_plus_extra = reference.fetch(
            ...
            molecule.spanEnd + extra_bp).upper()
        return span_plus_extra

    except ValueError as e:
        print(f'Error: {e} at position {molecule.chromosome}:{molecule.spanStart}-{molecule.spanEnd}. XXXX used as replacement.')
        return 'X'*1000
...
class Motifs:
    ...
    def get_feature_table(self):
        ...
        for i, molecule in enumerate(self.molecules):
            ...
            frag_seq = get_fragment_neighbouring_sequence(molecule, self.bp_count, self.reference)
...
def run(prepared_bam, reference, table=None, out_path="../test_out.tsv"):
    ...
    motifs = Motifs(molecules, bp_count=4, reference=reference, table=table, )
