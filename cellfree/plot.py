...
def plot_peaks_prominence_compare(rl_lists,
                                  filename,
                                  namelist,
                                  xmin=30,
                                  xmax=1000,
    ...
    for k, (cont_x, cont_y) in enumerate(cont_xy_list):
        ...
        plt.plot(peaks[0] + xmin, density_data[peaks[0]], "x",
                 label=f"{name}: {'bp, '.join([str(x + xmin) for x in peaks[0][:10]])}bp", color=f"dark{color}")
    ...
    plt.savefig(f"{outpath}/{filename}_{prominence_param}_{xmin}_{xmax}_density{density}_prominence_peaks_xmax{xmax}.png", dpi=80)
    #plt.show()

    return cont_xy_list


def plot_length_multi(bamfile_list, filename, outpath="../", density = False, len_range=[10, 1500], histtype = 'step', alpha=0.7):
    ...
    for name, file in bamfile_list:
        ...
        histogram = plt.hist(read_lengths ,
                 range=len_range,
                 label = name ,
...
parser = argparse.ArgumentParser(
    description='Plot the fragment length comparison')
parser.add_argument(
    'filepaths',nargs='+', type=str,
    help='profile file path to plot')
parser.add_argument(
    '--out_path', type=str, default="./",
    help='output path')
parser.add_argument(
    '--tickstep', type=int, default=100,
    help='tickstep')
parser.add_argument(
    '--xmin', type=int, default=10,
    help='min plotting range')
parser.add_argument(
    '--xmax', type=int, default=800,
    help='max plotting range')
...
rl_list2, histogram2 = plot_length_multi(bamfilelist,
                  f"{''.join(plotnames)}",
...
cont_xy = plot_peaks_prominence_compare(rl_list2, f"{'_'.join(filenames)}", plotnames , xmin=args.xmin ,xmax = args.xmax, tickstep=args.tickstep, outpath=args.out_path)
cont_xy = plot_peaks_prominence_compare(rl_list2, f"{'_'.join(filenames)}", plotnames ,density=False, xmin=args.xmin ,xmax = args.xmax, tickstep=args.tickstep, outpath=args.out_path)
