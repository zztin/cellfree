...
def plot_forward_length(length_counter, png, sample_name, dpi=100, start=0, end=1000):
    ...
    plt.plot([x for (x,y) in zip_rl], [y for (x,y) in zip_rl], label=sample_name, color='b')
    plt.xlim(start, end)
...
def plot_reverse_length(length_counter,
                        png,
                        sample_name,
                        dpi=100,
                        start=0,
    ...
    zip_rl = sorted(length_counter.items())
    plt.cla()
    plt.plot([x for (x,y) in zip_rl], [y for (x,y) in zip_rl], label=sample_name, color='black')
    plt.xlim(start, end)
    plt.xticks(range(start, end + 10, 100))
    plt.legend()
