...
def detect_alleles(
    ...
    return [
        allele
        for allele, cells in observed_alleles.items()
        if len(cells) >= min_cell_obs
    ]


def get_variant_phase(
    molecules,
    contig,
    position,
    variant_base,
    allele_resolver,
    phasing_ratio_threshold=None,
):  # (location,base) -> [( location, base, idenfifier)]
    ...
    if phasing_ratio_threshold is not None:
        ...
        if correct / total < phasing_ratio_threshold:
            raise ValueError(
                f"Phasing ratio not met. ({phasing_ratio}) < {phasing_ratio_threshold}"
            )
...
def molecule_to_random_primer_dict(
    molecule, primer_length=6, primer_read=2, max_N_distance=0
):
    rp = defaultdict(list)
...
class Molecule:
    ...
    def __init__(
        ...
        mapability_reader: typing.Optional[
            singlecellmultiomics.bamProcessing.MapabilityReader
        ] = None,
        allele_resolver: typing.Optional[
            singlecellmultiomics.alleleTools.AlleleResolver
        ] = None,
    ...
    def allele(self):
        ...
        raise NotImplementedError(
            f"allele_assingment_method {self.allele_assingment_method} is not defined"
        )
    ...
    def calculate_consensus(
        self, consensus_model, molecular_identifier, out, **model_kwargs
    ):
        """
    ...
    def get_consensus_read(
        ...
        if isinstance(consensus, str):
            ...
            sequence = "".join(
                (
                    consensus.get((self.chromosome, ref_pos), "N")
        ...
        if isinstance(phred_scores, dict):
            phred_score_array = list(
                phred_scores.get((self.chromosome, ref_pos), 0)
                for ref_pos in range(self.spanStart, self.spanEnd + 1)
            )
        else:
    ...
    def deduplicate_to_single(self, target_bam, read_name, classifier, reference=None):
        ...
        phred_scores = np.rint(
            -10
            * np.log10(np.clip(1 - base_calling_probs.max(1), 0.000000001, 0.999999))
        ).astype("B")
        ...
        **feature_matrix_args,
        ...
        if classifier is not None:
            (
                ...
                alignment_start,
                alignment_end,
            ) = self.get_base_calling_feature_matrix_spaced(
            ...
            phred_scores = np.rint(
                -10
                * np.log10(
                    np.clip(1 - base_calling_probs.max(1), 0.000000001, 0.999999)
                )
            ).astype("B")
    ...
    def extract_stretch_from_dict(self, base_call_dict, alignment_start, alignment_end):
        base_calling_probs = np.array(
            [
                base_call_dict.get((self.chromosome, pos), ("N", 0))[1]
                for pos in range(alignment_start, alignment_end)
            ]
        )
        predicted_sequence = [
            base_call_dict.get((self.chromosome, pos), ("N", 0))[0]
            for pos in range(alignment_start, alignment_end)
        ]
        predicted_sequence = "".join(predicted_sequence)
        phred_scores = np.rint(
            -10 * np.log10(np.clip(1 - base_calling_probs, 0.000000001, 0.999999999))
        ).astype("B")
    ...
    def deduplicate_majority(self, target_bam, read_name, max_N_span=None):
        ...
        reads = list(
            self.get_dedup_reads(
                read_name,
                target_bam,
                obs={
                    reference_position: phredscores_to_base_call(probs)
    ...
    def generate_partial_reads(self, obs, max_N_span=None):
        ...
        for operation, amount in CIGAR:
            if operation == "D":
                ...
                predicted_sequence, phred_scores = self.extract_stretch_from_dict(
                    obs, start_fetch, reference_end
                )  # [start .. end)
    ...
    def get_dedup_reads(self, read_name, target_bam, obs, max_N_span=None):
        ...
        for (
            ...
            partial_CIGAR,
            partial_MD,
        ) in self.generate_partial_reads(obs, max_N_span=max_N_span):
            consensus_read = self.get_consensus_read(
                read_name=read_name,
                target_file=target_bam,
                ...
                mdstring=create_MD_tag(
                    self.reference.fetch(
                        self.chromosome, reference_start, reference_end
                    ),
    ...
    def deduplicate_to_single_CIGAR_spaced_from_dict(
        ...
        for operation, amount in CIGAR:
            ...
            if operation == "M":  # Consume query and reference
                ...
                predicted_sequence, phred_scores = self.extract_stretch_from_dict(
                    base_call_dict, reference_start, reference_end
                )  # [start .. end)
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            ...
            for rt_id, fragments in self.get_rt_reactions().items():
                ...
                for fragment in fragments:
                    for read in fragment:
                        ...
                        if (
                            read is None
                            or read.reference_start > (end + 1)
                            or read.reference_end < start
                        ):
                            continue
                        for (
                            cycle,
                            ...
                            ref_pos,
                            ref_base,
                        ) in pysamiterators.ReadCycleIterator(
                            ...
                            if USE_RT:
                                if not (ref_pos, query_base) in RT_reaction_coverage:
                                    features[row_index][
                                        RT_INDEX
                                        + COLUMN_OFFSET
                                        + features_per_block * block_index
                                    ] += 1
                            ...
                            features[row_index][
                                ...
                                + COLUMN_OFFSET
                                + features_per_block * block_index
                            ] += abs(fragment.span[1] - fragment.span[2])
                            ...
                            if return_ref_info:
                                ...
                                if (
                                    row_index_in_output < 0
                                    or row_index_in_output
            ...
            for block_index in range(BASE_COUNT):  # ACGTN
                for index in (
                    ...
                    features[
                        :, index + COLUMN_OFFSET + features_per_block * block_index
                    ] /= features[
                        :, RC_INDEX + COLUMN_OFFSET + features_per_block * block_index
                    ]
    ...
    def get_base_calling_feature_matrix_spaced(
        self, return_ref_info=False, reference=None, **feature_matrix_args
    ):
        """
    ...
    def get_base_calling_training_data(
        ...
        **feature_matrix_args,
        ...
        use_indices = [
            mask_variants is None
            or not might_be_variant_function(chrom, pos, mask_variants, base)
            for chrom, pos, base in feature_info
        ]

        X_molecule = features[use_indices]
        y_molecule = [
            base for use, (chrom, pos, base) in zip(use_indices, feature_info) if use
        ]
    ...
    def is_valid(self, set_rejection_reasons=False):
        ...
        if (
            self.min_max_mapping_quality is not None
            and self.get_max_mapping_qual() < self.min_max_mapping_quality
        ):
            if set_rejection_reasons:
    ...
    def get_consensus_base_frequencies(self, allow_N=False):
        ...
        """
        return Counter(self.get_consensus(allow_N=allow_N).values())
    ...
    def __repr__(self):
        ...
        frag_repr = "\n\t".join(
            [
                textwrap.indent(str(fragment), " " * 4)
        ...
        """
            ...
            + (
                ""
                if len(self.fragments) < max_show
                else f"... {len(self.fragments) - max_show} fragments not shown"
            )
        )
    ...
    def _add_fragment(self, fragment):
        ...
        if self.max_associated_fragments is not None and len(self.fragments) >= (
            self.max_associated_fragments
        ):
        ...
        # else:
        self.spanStart = (
            add_span[1] if self.spanStart is None else min(add_span[1], self.spanStart)
        )
        self.spanEnd = (
            add_span[2] if self.spanEnd is None else max(add_span[2], self.spanEnd)
        )
    ...
    def is_completely_matching(self) -> bool:
        ...
        return all(
            (
                all([(operation == 0) for operation, amount in read.cigartuples])
                for read in self.iter_reads()
            )
        )
    ...
    def can_be_yielded(self, chromosome, position):
        ...
        if chromosome != self.chromosome:
            return True
        return position < (self.spanStart - self.cache_size * 0.5) or position > (
            self.spanEnd + self.cache_size * 0.5
        )
    ...
    def get_rt_reaction_fragment_sizes(self, max_N_distance=1):
        ...
        (
            ...
            frag_start,
            frag_end,
        ) = pysamiterators.iterators.getListSpanningCoordinates(
            ...
            (
                ...
                itertools.chain.from_iterable(
                    [
                        ...
                        and fragment.get_random_primer_hash()[0] is not None
                    ]
                )
            )
    ...
    def get_mean_rt_fragment_size(self):
        ...
        Returns:
            mean_rt_size (float)
        """
        return np.nanmean(self.get_rt_reaction_fragment_sizes())
    ...
    def write_pysam(
        ...
        consensus_name=None,
        consensus_read_callback=None,
        consensus_read_callback_kwargs=None,
    ):
        ...
        """
        if consensus:
            reads = self.deduplicate_majority(
                target_file,
    ...
    def set_methylation_call_tags(
        ...
        molecule_XM = Counter(
            list(d.get("context", ".") for d in self.methylation_call_dict.values())
        )
        # Contruct XM strings
        if reads is None:
        ...
        for read in reads:
            ...
            bis_met_call_string = "".join(
                [
                    call_dict.get((read.reference_name, rpos), {}).get("context", ".")
            ...
            read.set_tag(total_methylated_CPG_tag, molecule_XM["Z"])

            read.set_tag(total_unmethylated_CPG_tag, molecule_XM["z"])

            # Set total CHG methylated and unmethylated:
            read.set_tag(total_methylated_CHG_tag, molecule_XM["X"])

            read.set_tag(total_unmethylated_CHG_tag, molecule_XM["x"])

            # Set total CHH methylated and unmethylated:
            read.set_tag(total_methylated_CHH_tag, molecule_XM["H"])

            read.set_tag(total_unmethylated_CHH_tag, molecule_XM["h"])
    ...
    def get_mean_cycle(self, chromosome, position, base=None, not_base=None):
        ...
        return (np.mean(cycles_R1) if len(cycles_R1) else np.nan), (
            np.mean(cycles_R2) if len(cycles_R2) else np.nan
        )
    ...
    def base_likelihoods(self):
        return {
            (chrom, pos): base_probabilities_to_likelihood(probs)
            for (chrom, pos), probs in self.base_confidences.items()
        }
    ...
    def calculate_allele_likelihoods(self):
        ...
        for (
            chrom,
            pos,
        ), base_probs in self.allele_informative_base_probabilities.items():
    ...
    def get_allele(
        self, allele_resolver=None, return_allele_informative_base_dict=False
    ):
        """Obtain the allele(s) this molecule maps to
        ...
        try:
            for (chrom, pos), base in self.get_consensus(
                base_obs=self.get_base_observation_dict_NOREF()
            ).items():
    ...
    def write_allele_phasing_information_tag(
        ...
        if not use_likelihood:
            ...
            phase_str = "|".join(
                [
                    f"{chromosome},{position},{base},{allele}"
                    for allele, chromosome, position, base in phased_locations
                ]
            )
        else:
            ...
            phase_str = "|".join(
                [
                    f"{chromosome},{position},{base},{allele},{ prob_to_phred(confidence) }"
                    for allele, chromosome, position, base, confidence in phased_locations
                ]
            )
        ...
        for fragment in self:
            ...
            for read in fragment:
                ...
                for (
                    ...
                    query_pos,
                    ref_pos,
                ) in pysamiterators.iterators.ReadCycleIterator(read, with_seq=False):
    ...
    def get_base_observation_dict(
        self,
        return_refbases=False,
        allow_N=False,
        allow_unsafe=True,
        one_call_per_frag=False,
        min_cycle_r1=None,
        max_cycle_r1=None,
        min_cycle_r2=None,
        max_cycle_r2=None,
            ...
            for read in fragment:
                ...
                if allow_unsafe:
                    for query_pos, ref_pos, ref_base in read.get_aligned_pairs(
                        ...
                        if (
                            query_pos is None or ref_pos is None
                        ):  # or ref_pos < start or ref_pos > end:
                            continue
                        ...
                        if (
                            min_bq is not None
                            and read.query_qualities[query_pos] < min_bq
                        ):
                            continue
                    ...
                    for (
                        ...
                        ref_pos,
                        ref_base,
                    ) in pysamiterators.iterators.ReadCycleIterator(
                        ...
                        if (not read.is_paired or read.is_read1) and (
                            (min_cycle_r1 is not None and cycle < min_cycle_r1)
                            or (max_cycle_r1 is not None and cycle > max_cycle_r1)
                        ):
                            continue

                        if (read.is_paired and read.is_read2) and (
                            (min_cycle_r2 is not None and cycle < min_cycle_r2)
                            or (max_cycle_r2 is not None and cycle > max_cycle_r2)
                        ):
                            continue
    ...
    def get_match_mismatch_frequency(self, ignore_locations=None):
        ...
        for location, obs in base_obs.items():
            ...
            if location in ref_bases:
                ...
                matches += obs[ref]
                mismatches += sum(
                    (base_obs for base, base_obs in obs.most_common() if base != ref)
                )
    ...
    def get_consensus(
        ...
        **get_consensus_dictionaries_kwargs,
        ...
        proper = (
            v == v[np.arange(v.shape[0]), majority_base_indices][:, np.newaxis]
        ).sum(1) == 1
        ...
        if with_probs_and_obs:
            return (
                dict(
                    zip(
                        locations[proper],
                        ["ACGTN"[idx] for idx in majority_base_indices[proper]],
                    )
                ),
                phred_scores,
                consensii,
    ...
    def get_consensus_old(
        ...
        if classifier is not None:
            ...
            if (
                reuse_cached_consensus
                and hasattr(self, "classifier_consensus")
                and self.classifier_consensus is not None
            ):
            ...
            (
                ...
                alignment_start,
                alignment_end,
            ) = self.get_base_calling_feature_matrix_spaced(True)
            ...
            phred_scores = np.rint(
                -10
                * np.log10(
                    np.clip(1 - base_calling_probs.max(1), 0.000000001, 0.999999999)
                )
            ).astype("B")

            consensus = {
                (chrom, pos): consensus_base
                for (chrom, pos, ref_base), consensus_base in zip(
                    reference_bases, predicted_sequence
                )
            }
        ...
        for fragment in self:
            ...
            for read in fragment:
                ...
                for (
                    ...
                    query_pos,
                    ref_pos,
                ) in pysamiterators.iterators.ReadCycleIterator(read):
    ...
    def get_html(
        ...
        for location, query_base in consensus.items():
            try:
                if (
                    reference_bases is None
                    or reference_bases.get(location, "?") == query_base
                ):
                    visualized[location[1] - self.spanStart] = query_base
                    if reference_bases is not None:
                        # or reference_bases.get(location,'?')
                        reference_vis[location[1] - self.spanStart] = query_base
                else:
                    visualized[location[1] - self.spanStart] = style_str(
                        query_base, color="red", weight=800
                    )
                    if reference_bases is not None:
                        reference_vis[location[1] - self.spanStart] = style_str(
                            reference_bases.get(location, "?"),
                            color="black",
                            weight=800,
                        )
                ...
                for qpos, rpos, ref_base in read.get_aligned_pairs(with_seq=True):
                    ...
                    if methylation_status.isupper():
                        ...
                        if methylation_status == ".":
                            ...
                            if (
                                methylated_state.get((read.reference_name, rpos), 0)
                                == 0
                            ):
