...
class MoleculeIterator:
    ...
    Example:
        ...
        >>> from cellfree.molecule import MoleculeIterator
        >>> from cellfree.fragment import Fragment
        >>> import pysam
    ...
    def __iter__(self):
        ...
        for iteration, reads in enumerate(self.matePairIterator):
            ...
            if isinstance(reads, pysam.AlignedSegment):
                ...
                R1, R2 = reads
            elif (isinstance(reads, list) or isinstance(reads, tuple)) and len(
                reads
            ) == 1:
                R1 = reads[0]
                R2 = None
            ...
            if len(self.skip_contigs) > 0:
                ...
                for read in reads:
                    if (
                        read is not None
                        and read.reference_name not in self.skip_contigs
                    ):
                        keep = True
                if not keep:
                    continue
            ...
            if self.min_mapping_qual is not None:
                ...
                for read in reads:
                    if (
                        read is not None
                        and read.mapping_quality < self.min_mapping_qual
                    ):
            ...
            if (
                ...
                and self.waiting_fragments > self.max_buffer_size
            ):
                raise MemoryError(
                    f"max_buffer_size exceeded with {self.waiting_fragments} waiting fragments"
                )
                # modify can be yield func if max buffer size exceeded

            if (
                self.check_eject_every is not None
                and self.check_ejection_iter > self.check_eject_every
            ):
