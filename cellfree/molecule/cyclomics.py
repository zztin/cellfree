...
class CHICMolecule(Molecule):
    ...
    def _add_fragment(self, fragment):
        ...
        if fragment.site_location is not None:
            ...
            if self.site_location is None:
                self.site_location = [
                    fragment.site_location[0],
                    fragment.site_location[1],
                ]

            elif fragment.strand:  # Reverse:
                self.site_location[1] = max(
                    fragment.site_location[1], self.site_location[1]
                )  # this is the coordinate
            else:
                self.site_location[1] = min(
                    fragment.site_location[1], self.site_location[1]
                )  # this is the coordinate
    ...
    def write_tags(self):
        ...
        if (
            self.assignment_radius is not None
            and self.assignment_radius > 0
            and len(self) > 1
        ):
            for frag in self:
...
class CHICNLAMolecule(Molecule):
    ...
    def get_upstream_site(self, scan_extra_bp=3):
        ...
        if self.strand == 0:
            ...
            return self.reference.fetch(
                self.chromosome, start - 4 - scan_extra_bp, start
            )
        else:
            start = read.reference_end
            return self.reference.fetch(
                self.chromosome, start, start + 4 + scan_extra_bp
            )
